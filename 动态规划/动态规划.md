...
function process(arr, index, aim) {
    ...
    if (index === arr.length) {
        ...
        for (let zhang = 0; arr[index] * zhang <= aim; zhang++) {
...
```javascript
/**
    给定一个步数的数组，每个位置代表能走多少步，返回接一个结果，能走到最后位置最少需要多少步，不能走到则返回 -1
*/
function getMinStep(arr) {
    ...
    while (k >= 0) {
        ...
        if (k + arr[k] >= i && dp[k] + 1 < dp[i]) {
        dp[i] = dp[k] + 1
        }
        k--
    }
    dp[i] = dp[i] === Number.MAX_SAFE_INTEGER ? -1 : dp[i]
    }
    console.log(dp)
    return dp[arr.length - 1]
}
getMinStep([0, 5, 0, 7, 1, 3, 2, 0, 0])
...
```javascript
// 暴力递归解
// n: 物品个数 w: 背包容量 wt: 物品重量 val: 物品价值
function pack(n, w, wt, val) {
    function exec(w, step, v) {
        if (step == n) {
            return v
        }
        if (w - wt[step] >= 0) {
            return Math.max(
            exec(w - wt[step], step + 1, v + val[step]),
            exec(w, step + 1, v)
            )
        } else {
            return exec(w, step + 1, v)
        }
    }
    return exec(w, 0, 0)
}
// dp 放与不放问题
function dpExec(n, w, wt, val) {
...
## 子集背包问题
给一个集合，求其是否可以将其划分为两个和相等的集合
```javascript
function canPartition(arr) {
    const sum = arr.reduce((pre, cur) => cur + pre, 0)
    if (sum % 2 == 1) return false
    const w = sum / 2,
        dp = new Array(arr.length + 1)
    for(let i = 0; i < dp.length; i++) {
        if(i === 0) {
...
##  编辑距离问题
给定 s1 和 s2 两个字符串，求将 s1 变为 s2 所需要的最小步数是多少
```javascript
function minDis(s1, s2) {
    function exec(i, j) {
        if(i === -1) return j + 1
        if(j === -1) return i + 1
        if(s1[i] === s2[j]) return exec(i - 1, j - 1)
        return Math.min(
            exec(i, j - 1) + 1, // 插入
            exec(i - 1, j - 1) + 1, // 替换
            exec(i - 1, j) + 1 // 删除
    ...
    return exec(s1.length - 1, s2.length - 1)
}
function minDisDp(s1, s2){
